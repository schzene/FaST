...
inline double gelu_tanh(double dbl_x) {
    return 0.5 * dbl_x *
           (1 +
            tanh(sqrt(2 / M_PI) * (dbl_x + 0.044715 * dbl_x * dbl_x * dbl_x)));
}

// if x <= -5.075 : seg5gelu = 0
// if - 5.075 < x and x <= -1.414 : #poor
// seg5gelu = -0.568686678 - 0.529288810 *x - 0.183509590 *x ** 2 - 0.028070202
// *x ** 3 - 0.001597741 *x ** 4 if - 1.414 < x and x < 1.414 : #good ! seg5gelu
// = 0.001193207 + 0.5 *x + 0.385858026 *x ** 2 + .0 *x ** 3 - 0.045101361 *x **
// 4 if 1.414 <= x and x < 5.075 : #bad seg5gelu = -0.438406187 + 1.340789252 *x
// - 0.087184212 *x ** 2 + 0.007334718 *x ** 3 if x >= 5.075 : seg5gelu = x
...
inline double approx_gelu(double x) {
    ...
    if (x < -1.414) {
        return -0.568686678 - 0.529288810 * x - 0.183509590 * x2 -
               0.028070202 * x3 - 0.001597741 * x4;
    }
    if (x < 1.414) {
        return 0.001193207 + 0.5 * x + 0.385858026 * x2 - 0.045101361 * x4;
    }
    if (x < 5.072) {
        return -0.438406187 + 1.340789252 * x - 0.087184212 * x2 +
               0.007334718 * x3;
    }
    return x;
}
...
class SecureFFN {
    ...
    LongCiphertext f2(const LongCiphertext &x1, const LongCiphertext &x2,
    ...
    LongCiphertext f1(const LongCiphertext &x1, const LongCiphertext &x2,
                      const LongCiphertext &x3, const LongCiphertext &x4) {
        LongPlaintext parm0(-0.568686678, encoder),
            parm1(-0.529288810, encoder), parm2(-0.183509590, encoder),
            parm3(-0.028070202, encoder), parm4(-0.001597741, encoder);
        parm4.mod_switch_to_inplace(x4.parms_id(), evaluator);
        parm3.mod_switch_to_inplace(x3.parms_id(), evaluator);
        parm2.mod_switch_to_inplace(x2.parms_id(), evaluator);
        parm1.mod_switch_to_inplace(x1.parms_id(), evaluator);
...
public:
    SecureFFN(CKKSKey *alice_, CKKSKey *bob_, CKKSEncoder *encoder_,
    ...
    void forward(const LongCiphertext &ln_secret_a) {
        ...
        LongCiphertext xa_secret_a =
            ln_secret_a.add_plain(neg_xb_plain, evaluator);
        ...
        LongCiphertext B1a_secret_a(B1a_plain, alice),
            div_va_secret_a(div_va_plain, alice);
        ...
        LongPlaintext tmp11_plain(tmp11, encoder),
            xb_W1b_plain(xb_W1b, encoder), B1b_plain(B1b, encoder);
        ...
        LongPlaintext parm0(5.075, encoder), parm1(sqrt(2), encoder),
            parm2(-sqrt(2), encoder), parm3(-5.075, encoder);
        ...
        matrix s0_sgn_b(batch_size * ffn_dim), s1_sgn_b(batch_size * ffn_dim),
            s2_sgn_b(batch_size * ffn_dim), s3_sgn_b(batch_size * ffn_dim);
        random_mat(s0_sgn_b, -1, 1, true);
        random_mat(s1_sgn_b, -1, 1, true);
        random_mat(s2_sgn_b, -1, 1, true);
        random_mat(s3_sgn_b, -1, 1, true);
        LongPlaintext s0_sgn_plain_b(s0_sgn_b, encoder),
            s1_sgn_plain_b(s1_sgn_b, encoder),
            s2_sgn_plain_b(s2_sgn_b, encoder),
            s3_sgn_plain_b(s3_sgn_b, encoder);
        ...
        // send s0_sgn_a, s1_sgn_a, s2_sgn_a, s3_sgn_a, rs_rc_x1, x1_secret_a_,
        // x1_2_secret_a_, x1_3_secret_a_, x1_4_secret_a_ to bob;
        ...
        matrix b1(batch_size * ffn_dim), b2(batch_size * ffn_dim),
            b3(batch_size * ffn_dim), b4(batch_size * ffn_dim);
        matrix x1_2(batch_size * ffn_dim), x1_3(batch_size * ffn_dim),
            x1_4(batch_size * ffn_dim);
        for (i = 0; i < batch_size * ffn_dim; i++) {
         ...
         *
         */
        // double rs_f4 = dist(gen), rs_f3 = dist(gen), rs_f2 = dist(gen), rs_f1
        // = dist(gen); LongPlaintext rs_f4_plain(rs_f4, encoder),
        // rs_f3_plain(rs_f3, encoder), rs_f2_plain(rs_f2, encoder),
        // rs_f1_plain(rs_f1, encoder); LongPlaintext neg_rs_f4_plain(-rs_f4,
        // encoder), neg_rs_f3_plain(-rs_f3, encoder), neg_rs_f2_plain(-rs_f2,
        // encoder), neg_rs_f1_plain(-rs_f1, encoder);
        ...
        LongPlaintext f4_plain = f4_.decrypt(alice),
                      f3_plain = f3_.decrypt(alice),
                      f2_plain = f2_.decrypt(alice),
                      f1_plain = f1_.decrypt(alice);
        ...
        LongCiphertext _f4_(_f4_plain, alice), _f3_(_f3_plain, alice),
            _f2_(_f2_plain, alice), _f1_(_f1_plain, alice);
        ...
        matrix v1a_x1a_W2a =
            matmul(v1a_x1a, W2a, batch_size, ffn_dim, d_module);
        for (i = 0; i < ffn_dim * d_module; i++) {
            W2a[i] *= v1a;
        }
        LongPlaintext B2a_plain(B2a, encoder), div_v1a_plain(1. / v1a, encoder);
        LongCiphertext B2a_secret_a(B2a_plain, alice),
            div_v1a_secret_a(div_v1a_plain, alice);
        ...
        LongPlaintext tmp21_plain(tmp21, encoder),
            x1b_W2b_plain(x1b_W2b, encoder), B2b_plain(B2b, encoder);
...
int main() {
    ...
    for (int i = 0; i < dim; ++i) {
        ...
        double gelu_y = 0.5 * x1[i] *
                        (1 + tanh(sqrt(2 / M_PI) *
                                  (x1[i] + 0.044715 * x1[i] * x1[i] * x1[i])));
    }
    STOP_TIMER("gelu")
    START_TIMER
