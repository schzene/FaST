...
public:
    ...
    void forward(BFVLongCiphertext &attn_s, const bfv_matrix &input_a, const bfv_matrix &input_b) {
        ...
        BFVLongPlaintext ha2_plain(bfv_parm, fix_ha2, encoder);
        ...
        BFVLongPlaintext input_b_plain(bfv_parm, input_b, encoder);
        ...
        BFVLongPlaintext ha1_xa_plain(bfv_parm, ha1_xa.data, ha1_xa.size, encoder); // error:  fixrry need to mod to plain_mod
...
int main() {
    ...
    BFVKey *bob = new BFVKey(sci::BOB, bfv_parm->context);

    BatchEncoder *encoder = new BatchEncoder(*bfv_parm->context);

    Evaluator *evaluator = new Evaluator(*bfv_parm->context);
    ...
    sci::IOPack *iopack;
    sci::OTPack *otpack;
    SecureLayerNorm1 *sec_ln1 = new SecureLayerNorm1(bfv_parm, alice, bob, encoder, evaluator, iopack, otpack);
