...
LongCiphertext FFN::f3(const LongCiphertext &x1, const LongCiphertext &x2, const LongCiphertext &x3) {
    LongPlaintext parm0(-0.438406187, encoder), parm1(1.340789252, encoder), parm2(-0.087184212, encoder),
        parm3(0.007334718, encoder);
    ...
    LongCiphertext f3_ = x3.multiply_plain(parm3, evaluator), tmp1 = x2.multiply_plain(parm2, evaluator),
...
LongCiphertext FFN::f2(const LongCiphertext &x1, const LongCiphertext &x2, const LongCiphertext &x4) {
    LongPlaintext parm0(0.001193207, encoder), parm1(0.5, encoder), parm2(0.385858026, encoder),
        parm4(-0.045101361, encoder);
    parm4.mod_switch_to_inplace(x4.parms_id(), evaluator);
    parm2.mod_switch_to_inplace(x2.parms_id(), evaluator);
    parm1.mod_switch_to_inplace(x1.parms_id(), evaluator);
    LongCiphertext f2_ = x4.multiply_plain(parm4, evaluator), tmp2 = x2.multiply_plain(parm2, evaluator),
...
LongCiphertext FFN::f1(const LongCiphertext &x1, const LongCiphertext &x2, const LongCiphertext &x3,
                       const LongCiphertext &x4) {
    LongPlaintext parm0(-0.568686678, encoder), parm1(-0.529288810, encoder), parm2(-0.183509590, encoder),
        parm3(-0.028070202, encoder), parm4(-0.001597741, encoder);
    parm4.mod_switch_to_inplace(x4.parms_id(), evaluator);
    parm3.mod_switch_to_inplace(x3.parms_id(), evaluator);
    parm2.mod_switch_to_inplace(x2.parms_id(), evaluator);
    parm1.mod_switch_to_inplace(x1.parms_id(), evaluator);
    LongCiphertext f1_ = x4.multiply_plain(parm3, evaluator), tmp1 = x3.multiply_plain(parm3, evaluator),
                   tmp2 = x2.multiply_plain(parm2, evaluator), tmp3 = x1.multiply_plain(parm1, evaluator);
...
LongCiphertext FFN::forward(const LongCiphertext &ln1) {
    ...
    if (party->party == sci::ALICE) {
        ...
        double div_rc = 1. / rc, div_rc_2 = div_rc * div_rc, div_rc_3 = div_rc_2 * div_rc,
               div_rc_4 = div_rc_2 * div_rc_2;
        LongPlaintext B1_plain(B1, encoder), div_va_plain(1. / va, encoder), div_rc_plain(div_rc, encoder),
            div_rc_2_plain(div_rc_2, encoder), div_rc_3_plain(div_rc_3, encoder), div_rc_4_plain(div_rc_4, encoder),
            B2a_plain(B2, encoder), div_v1a_plain(1. / v1a, encoder);
        LongCiphertext x_secret_a, B1_secret_a(B1_plain, party), div_va_secret_a(div_va_plain, party), s0_sgn_secret_a,
            s1_sgn_secret_a, s2_sgn_secret_a, s3_sgn_secret_a, x1_secret_a, div_rc_secret_a(div_rc_plain, party),
            div_rc_2_secret_a(div_rc_2_plain, party), div_rc_3_secret_a(div_rc_3_plain, party),
            div_rc_4_secret_a(div_rc_4_plain, party), f4_, f3_, f2_, f1_, gelu_secret_a, B2a_secret_a(B2a_plain, party),
        ...
        LongPlaintext s0_sgn_plain_a = s0_sgn_secret_a.decrypt(party), s1_sgn_plain_a = s1_sgn_secret_a.decrypt(party),
                      s2_sgn_plain_a = s2_sgn_secret_a.decrypt(party), s3_sgn_plain_a = s3_sgn_secret_a.decrypt(party);
        matrix s0_sgn_a = s0_sgn_plain_a.decode(encoder), s1_sgn_a = s1_sgn_plain_a.decode(encoder),
               s2_sgn_a = s2_sgn_plain_a.decode(encoder), s3_sgn_a = s3_sgn_plain_a.decode(encoder);
        ...
        LongPlaintext f4_plain = f4_.decrypt(party), f3_plain = f3_.decrypt(party), f2_plain = f2_.decrypt(party),
                      f1_plain = f1_.decrypt(party);
        matrix f4_m = f4_plain.decode(encoder), f3_m = f3_plain.decode(encoder), f2_m = f2_plain.decode(encoder),
               f1_m = f1_plain.decode(encoder);
        LongPlaintext _f4_plain(f4_m, encoder), _f3_plain(f3_m, encoder), _f2_plain(f2_m, encoder),
            _f1_plain(f1_m, encoder);
        ...
        matrix x(batch_size * d_module), neg_x(batch_size * d_module), x1(batch_size * ffn_dim),
            neg_x1(batch_size * ffn_dim);
        random_mat(x);
        random_mat(x1);
        for (i = 0; i < batch_size * d_module; i++) {
            neg_x[i] = -x[i];
        ...
        matrix s0_sgn_a(batch_size * ffn_dim), s1_sgn_a(batch_size * ffn_dim), s2_sgn_a(batch_size * ffn_dim),
            s3_sgn_a(batch_size * ffn_dim), va_xa(batch_size * d_module), va_xa_W1a(batch_size * ffn_dim),
            W1a(d_module * ffn_dim), s0_sgn_b(batch_size * ffn_dim), s1_sgn_b(batch_size * ffn_dim),
            s2_sgn_b(batch_size * ffn_dim), s3_sgn_b(batch_size * ffn_dim), b1(batch_size * ffn_dim),
            b2(batch_size * ffn_dim), b3(batch_size * ffn_dim), b4(batch_size * ffn_dim), x1_2(batch_size * ffn_dim),
            x1_3(batch_size * ffn_dim), x1_4(batch_size * ffn_dim), rs_rc_x1(batch_size * ffn_dim),
        ...
        LongPlaintext neg_xb_plain(neg_x, encoder), parm0(5.075, encoder), parm1(sqrt(2), encoder),
            parm2(-sqrt(2), encoder), parm3(-5.075, encoder), s0_sgn_plain_b(s0_sgn_b, encoder),
            s1_sgn_plain_b(s1_sgn_b, encoder), s2_sgn_plain_b(s2_sgn_b, encoder), s3_sgn_plain_b(s3_sgn_b, encoder),
            rs_plain(rs, encoder), neg_x1b_plain(neg_x1, encoder), div_vb_plain(-vb, encoder);
        LongCiphertext B1a_secret_a, div_va_secret_a, x1_secret_a_, x1_2_secret_a_, x1_3_secret_a_, x1_4_secret_a_,
            _f1_, _f2_, _f3_, _f4_, B2a_secret_a, div_v1a_secret_a, vb_secret_b(vb, party, encoder);
#ifdef LOG
        INIT_TIMER
        START_TIMER
#endif
        ...
        LongPlaintext x1_plain(rs_rc_x1, encoder), x1_2_plain(x1_2, encoder), x1_3_plain(x1_3, encoder),
            x1_4_plain(x1_4, encoder);
