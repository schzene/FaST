...
LongCiphertext FFN::forward(const LongCiphertext &ln1)
{
    ...
    {
        LongCiphertext x_secret_a;
        double va = dist(gen);
        LongPlaintext B1_plain(B1, encoder), div_va_plain(1. / va, encoder);
        LongCiphertext B1_secret_a(B1_plain, party), div_va_secret_a(div_va_plain, party);
        LongCiphertext s0_sgn_secret_a, s1_sgn_secret_a, s2_sgn_secret_a, s3_sgn_secret_a, x1_secret_a;
        double rc = dist(gen);
        ...
        LongCiphertext div_rc_secret_a(div_rc_plain, party),
            div_rc_2_secret_a(div_rc_2_plain, party),
            div_rc_3_secret_a(div_rc_3_plain, party),
            div_rc_4_secret_a(div_rc_4_plain, party);
        LongCiphertext f4_, f3_, f2_, f1_;
        LongCiphertext gelu_secret_a;
        double v1a = dist(gen);
        LongPlaintext B2a_plain(B2, encoder), div_v1a_plain(1. / v1a, encoder);
        LongCiphertext B2a_secret_a(B2a_plain, party), div_v1a_secret_a(div_v1a_plain, party);
#ifdef LOG
        INIT_TIMER
        START_TIMER
#endif
        ...
        LongCiphertext::recv(io_pack, &f4_, party->context);
        LongCiphertext::recv(io_pack, &f3_, party->context);
        LongCiphertext::recv(io_pack, &f2_, party->context);
        LongCiphertext::recv(io_pack, &f1_, party->context);
        ...
        LongCiphertext::send(io_pack, &_f4_);
        LongCiphertext::send(io_pack, &_f3_);
        LongCiphertext::send(io_pack, &_f2_);
    ...
    else
    {
        ...
        matrix va_xa(batch_size * d_module), va_xa_W1a(batch_size * ffn_dim), W1a(d_module * ffn_dim);
        ...
        matrix s0_sgn_b(batch_size * ffn_dim), s1_sgn_b(batch_size * ffn_dim), s2_sgn_b(batch_size * ffn_dim), s3_sgn_b(batch_size * ffn_dim), rs_rc_x1(batch_size * ffn_dim);
        ...
        LongCiphertext x1_secret_a_, x1_2_secret_a_, x1_3_secret_a_, x1_4_secret_a_;
        LongCiphertext _f1_, _f2_, _f3_, _f4_;
        LongPlaintext neg_x1b_plain(neg_x1, encoder);
        matrix v1a_x1a(batch_size * ffn_dim), v1a_x1a_W2a(batch_size * d_module), W2a(ffn_dim * d_module);
        LongCiphertext B2a_secret_a, div_v1a_secret_a;
#ifdef LOG
        INIT_TIMER
        START_TIMER
#endif
        ...
        LongCiphertext::send(io_pack, &f4_);
        LongCiphertext::send(io_pack, &f3_);
        LongCiphertext::send(io_pack, &f2_);
        LongCiphertext::send(io_pack, &f1_);

        // bob recv _f4_, _f3_, _f2_, _f1_
        LongCiphertext::recv(io_pack, &_f4_, party->context);
        LongCiphertext::recv(io_pack, &_f3_, party->context);
        LongCiphertext::recv(io_pack, &_f2_, party->context);
        LongCiphertext::recv(io_pack, &_f1_, party->context);
        /**
         * end TODO1
         */
