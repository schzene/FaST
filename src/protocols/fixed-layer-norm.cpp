...
FixArray standard_deviation(const FixArray &input, const FixArray &means, size_t row, size_t column) {
    ...
    for (i = 0; i < row; i++) {
        for (j = 0; j < column; j++) {
            result.data[i] +=
                (input.data[i * column + j] - means.data[i]) * (input.data[i * column + j] - means.data[i]);
        }
        result.data[i] /= column;
...
BFVLongCiphertext FixedLayerNorm::forward(const BFVLongCiphertext &attn, const bfv_matrix &input) const {
    ...
    if (party->party == sci::ALICE) {
        ...
        const uint64_t fix_h2_div_h1 =
            sci::neg_mod(static_cast<int64_t>(ha2_div_ha1 * (1ULL << DEFAULT_BITWIDTH)), DEFAULT_ELL);
        const uint64_t fix_div_ha2 =
            sci::neg_mod(static_cast<int64_t>(div_ha2 * (1ULL << DEFAULT_BITWIDTH)), DEFAULT_ELL);

        FixArray fix_input = fixop->input(party->party, input.size(), input.data());
        BFVLongPlaintext ha2_plain(parm, fix_ha2);
        BFVLongCiphertext ha2_div_ha1_secret_a(parm, fix_h2_div_h1, party), ha2_secret_a(ha2_plain, party),
            xha1_secret_a;
        ...
        */
        ...
        for (i = 0; i < batch_size; i++) {
            for (j = 0; j < d_module; j++) {
                int64_t tmp0 =
                    (fix_xgb.data[i * d_module + j] - mu_gb.data[i]) * int64_t(ka * (1ULL << DEFAULT_BITWIDTH)) >>
                    DEFAULT_BITWIDTH;
                ...
                div_sigma_gb[i * d_module + j] =
                    sci::neg_mod(int64_t(tmp3 * (1ULL << DEFAULT_BITWIDTH)), DEFAULT_BITWIDTH);
            }
        }
