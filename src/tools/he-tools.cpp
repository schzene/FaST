...
LongPlaintext::LongPlaintext(Plaintext pt, size_t slot_count_, CKKSEncoder* encoder_): 
        slot_count(slot_count_),  encoder(encoder_) {
    len = 1;
    plain_data.push_back(pt);
}

LongPlaintext::LongPlaintext(std::vector<double> data, double scale, size_t slot_count_, CKKSEncoder* encoder_): 
        slot_count(slot_count_), encoder(encoder_) {
    len = data.size();
    size_t count = len / slot_count;
    if (len % slot_count) count++;
    size_t i, j;
    if (slot_count >= len) {
        ...
        while (curPtr < endPtr) {
            ...
            Plaintext pt; encoder->encode(temp, scale, pt);
            plain_data.push_back(pt);
            curPtr += slot_count;
        }
    }
...
std::vector<double> LongPlaintext::decode() {
    ...
    for (size_t i = 0; i < size; i++) {
        std::vector<double> temp; encoder->decode(plain_data[i], temp);
        if (i < size - 1)
...
void LongCiphertext::add_plain_inplace(LongPlaintext& lpt, Evaluator* evaluator) {
    if (len == 1) {
        ...
        for (Plaintext pt: lpt.plain_data) {
            Ciphertext ctemp; evaluator->add_plain(ct, pt, ctemp);
...
LongCiphertext LongCiphertext::multiply_plain(LongPlaintext& lpt, Evaluator* evaluator) {
    ...
    if (len == 1) {
        ...
        for (size_t i = 0; i < lpt.plain_data.size(); i++) {
            Ciphertext ct; evaluator->multiply_plain(cipher_data[0], lpt.plain_data[i], ct);
            lct.cipher_data.push_back(ct);
        }
    } else if(lpt.len == 1) {
        lct.len = len;
        for (size_t i = 0; i < cipher_data.size(); i++) {
            Ciphertext ct; evaluator->multiply_plain(cipher_data[i], lpt.plain_data[0], ct);
            lct.cipher_data.push_back(ct);
        }
    }else if (len == lpt.len) {
        lct.len = len;
        for (size_t i = 0; i < cipher_data.size(); i++) {
            Ciphertext ct; evaluator->multiply_plain(cipher_data[i], lpt.plain_data[i], ct);
            lct.cipher_data.push_back(ct);
        }
    }
    return lct;
...
void LongCiphertext::multiply_plain_inplace(LongPlaintext& lpt, Evaluator* evaluator) {
    if (len == 1) {
        ...
        cipher_data.pop_back();
        for (Plaintext pt: lpt.plain_data) {
            Ciphertext ctemp; evaluator->multiply_plain(ct, pt, ctemp);
            cipher_data.push_back(ctemp);
        }
    } else if(lpt.len == 1) {
        for (size_t i = 0; i < cipher_data.size(); i++)
