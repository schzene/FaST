...
#define STOP_TIMER(name)                                              \
    std::cout << "------------------------------------" << std::endl; \
    std::cout << "[STOPPING TIMER] Total RUNTIME of " << name << ": " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - start_timer).count() + pause_timer << " ms " << std::endl;
